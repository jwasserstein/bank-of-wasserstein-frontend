...
export function logIn(username, password) {
	return dispatch => {
		return new Promise(async (resolve, reject) => {
			try {
				const resp = await apiCall('post', 'http://localhost:3001/api/auth/signin', {username, password}, '');
...
export function signUp(username, email, password){
	return dispatch => {
		return new Promise(async (resolve, reject) => {
			try {
				const resp = await apiCall('post', 'http://localhost:3001/api/auth/signup', {username, email, password}, '');
				if(resp.error){
					return reject(resp.error);
				}
				if(!resp.token){
